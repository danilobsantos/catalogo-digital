...
<article {{ $attributes->merge(['class' => 'group relative flex flex-col overflow-hidden rounded-2xl bg-white border border-[#E6E1D5] hover:border-[#ff8400]/50 shadow-warm-xs hover:shadow-warm-md transition-all duration-300']) }}>
    <a href="{{ $href }}" class="block flex-1">
        <div class="aspect-square bg-[#F4F1EA] overflow-hidden relative">
            ...
            <div class="absolute top-3 left-3 flex flex-col gap-1.5 z-10">
                @if ($product->is_new)
                    <span class="rounded-full bg-[#A84D29] px-2.5 py-0.5 text-[10px] uppercase tracking-wider font-bold text-white shadow-xs">
                        Novo
                    </span>
                @endif
                @if ($product->has_ca)
                    <span class="rounded-full bg-[#544D42] px-2.5 py-0.5 text-[10px] uppercase tracking-wider font-bold text-white shadow-xs">
                        C.A.
                    </span>
                @endif
            </div>
        ...
        <div class="p-4 sm:p-5 pb-0">
            <p class="text-[11px] uppercase tracking-wider font-semibold text-[#ff8400]">
                Cód: {{ $product->code }}{{ $product->variant_code ? '-'.$product->variant_code : '' }}
                @if ($product->leather) · {{ $product->leather }} @endif
            </p>
            <h3 class="mt-1 font-display font-bold text-sm sm:text-base leading-snug line-clamp-2 text-[#1C1915] group-hover:text-[#ff8400] transition-colors">
                {{ $product->name }}
            </h3>
        </div>
    </a>

    <div class="p-4 sm:p-5 mt-auto pt-3">
        <a href="{{ $whatsapp }}" target="_blank" rel="noopener"
           class="w-full flex items-center justify-center gap-2 rounded-xl bg-[#047857] hover:bg-[#065F46] text-white py-2.5 px-4 text-xs font-bold uppercase tracking-wider transition-all duration-150 shadow-xs hover:shadow-sm active:scale-[0.98]">
